...
\begin{enumerate}
	...
	\item Su tali valori viene applicato un modello di \textit{Gaussian Mixture Model} (GMM) con $k$ componenti, 
		...
		\begin{enumerate}
			\item Per prima cosa vengono addestrati diversi GMM con un numero cresente di componenti, da $1$ a $K_{\text{max}}$, in modo da ottenere 
			...
			\item Ciascun modello viene poi valutato con una funzione $V: GMM \rightarrow \mathbb{R}$, (come ad esempio AIC, BIC, ecc...),
				...
				\end{equation}
			\item Infine, i valori ottenuti vengono rappresentati graficamente e il modello ottimale viene scelto nel \textit{Knee Point}, cioè dove l'incremento di qualità del modello diventa marginale.
...
\begin{itemize}
  ...
  \item \texttt{library\_id}: Opzionale, è l'dentificativo della libreria Visium. Risulta utile quando occorre concatenare più librerie.
...
Al contario, con $k \>> 3$ si verifica il problema opposto, ovvero si hanno troppe gaussiane per rappresentare efficacemente i dati in oggetto, 
...
\begin{lstlisting}[style=mypython,
				   ...
				   caption={Funzioni per ottenere le machere binarie una volta ottenuta la k del Knee Point},
				   basicstyle=\ttfamily\scriptsize]
	def fit_MoG(gdata, k):
		data = gdata.reshape(-1, 1)
