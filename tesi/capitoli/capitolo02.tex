...
\end{equation}
dove ogni $g_j$ è un gene.
Di conseguenza si può definire lo spot $s_i \in \mathbb{G}^{m}$ e assegniamo ad ogni 
$s_i$ il seguente vettore: 
\begin{equation}
	\bigl( x_{i,1}, x_{i,2}, \dots, x_{i,m} \bigr)
\end{equation}
dove $x_{i,j}\in\mathbb{R}$ rappresenta l'espressione del gene $g_j$
allo spot $s_i$, con $1 \le i \le n$ e $1 \le j \le m$.
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.575\textwidth]{immagini/spot_cubo.png}
	\caption{Rappresentazione spots ($s_i$) e geni (i vari colori)}
	\label{fig:four}
\end{figure}
...
Si può notare che ogni spot $s_i$, al suo interno, può contenere più geni.
Inoltre, nella figura d'esempio, si hanno 36 spot diversi e 4 geni, ma ma nella realtà i dati
sono differenti: gli spot possono arrivare a oltre 3.000
celle e i geni possono superare le 30.000 unità.
...
Questo significa che, dato uno spot $s_i$, questo presenterà sei spot
adiacenti, tranne nei casi in cui gli spot sono localizzati ai bordi del tessuto.

Per poter trovare spazialmente uno spot $s_i$, quindi, occorre assegnare una coordinata $x$ e una coordinata $y$ ad ogni spot, e, una volta creata una matrice \\$[s_i, <x_i, y_i>]$,
dove $<x, y>$ indicano le coordinate dello spot, è possibile trovare i 6 vicini attraverso un semplice calcolo.
Inoltre, per sopperire ad eventuali spazi vuoti nel tessuto, viene calcolata la distanza euclidea tra gli spot, in modo da poter scartare gli spot che altrimenti verrebbero erroneamente segnati come vicini.
...
\section{Preprocessing dei dati}\label{sec:capPreProcessing}
...
questo perchè i dati di trascrittomica spaziale contengono anche rumore tecnico e biologico che può mascherare dei pattern in realtà utili, come ad esempio dei geni con profili irregolari.
Occorre, quindi, prima di tutto, ripulire e ridurre la complessità dei dati prima di poterli analizzare.

Per lo studio che questa tesi si è posta come obiettivo, il preprocessing consiste in tre fasi principali:
\begin{enumerate}
	\item Smoothing Spaziale
	\item Partizionamento delle espressioni
	\item Creazione maschere binarie
\end{enumerate}
Queste 3 fasi sono fondamentali per ottenere per ciascun gene 
una lista dei vari spot dove il gene ha, a 
tutti gli effetti, una sua espressione e dove 
invece quest'ultima non è presente\cite{binaryMasksBonniciGrazioli}.
\subsection{Smoothing spaziale}\label{subsec:SpatialSmoothing}

Il primo step, ampiamente utilizzato in gran parte degli algoritmi di trascrittomica spaziale\cite{Giotto,SpatialSmoothing1,SpatialSmoothing2}, per poter poter minimizzare il rumore è applicare un \textit{filtro di media} (Mean Filter),
ovvero una tecnica per poter sostituire ogni elemento, con la media dei valori dei suoi vicini dato un raggio prestabilito.
...
Possiamo denotare con $y_{ij} \in \mathbb{R}$ il livello di espressione genica di un determinato gene $g_i$ nello spot $s_i$.
...
\medskip
\noindent
In alternativa al filtro di media è possibile utilizzare la \textit{Gaussian Kernel Smoothing}\cite{GaussianKernelSmoothing} 
per calcolare i pesi spaziali.
...
La figura \ref{fig:six} dimostra come il filtro di media attenua le variazioni locali, riducendo i valori e producendo una mappa più omogenea dove i picchi isolati di rumore risultano smussati.
...
\end{equation}
Nell'equazione \ref{eq:smoothingTwoFactors} ci sono due parametri, $\alpha$ e $\beta$.
Il primo regola il rapporto tra l'espressione originale e quella corretta, 
...
\subsection{Partizionamento delle espressioni}\label{subsec:ExpressionsPartitioning}
Una volta ottenuti i dati dei vari geni smussati $\hat{y}_i = (\hat{y}_{i1},...,\hat{y}_{in})$ dal filtro di media \ref{eq:filtroDiMedia}, 
il passo successivo consiste nel suddividere le posizioni spaziali associate a ciascun gene $g_i$ in sotto-regioni omogenee.

Formalmente per ogni gene $g_i \in \mathbb{G}$:
\begin{enumerate}
	\item I vari valori di espressione genica smussati $\{\hat{y}_{ij}\}_{j=1}^n$ vengono trattati 
		come osservazioni all'interno di uno spazio probabilistico.
	\item Su tali valori viene applicato un modello di \textit{Gaussian Mixture Model} (GMM) con $k$ componenti, 
		che consente di rappresentare la distribuzione dei dati come combinazioni di più distribuzioni gaussiane.
		Il numero di componenti $k$ del modello non viene scelto a priori, ma determinato in modo automatico secondo la seguente procedura:
		\begin{enumerate}
			\item Per prima cosa vengono addestrati diversi GMM con un numero crescente di componenti, da $1$ a $K_{\text{max}}$, in modo da ottenere 
				un set di modelli ordinati per numero di componenti:
				\begin{equation}
					\label{eq:GMMPrimoStep}
					\mathbb{M}_i = (GMM_1(\hat{y}_i), GMM_2(\hat{y}_i), ..., GMM_{K_{\text{max}}}(\hat{y}_i))
				\end{equation}
			\item Ciascun modello viene poi valutato con una funzione $V: GMM \rightarrow \mathbb{R}$, (come ad esempio AIC, BIC, ecc...),
				in modo da ottenere il vettore:
				\begin{equation}
					\label{eq:GMMSecondoStep}
					\mathbb{V}_i = (V(GMM_1(\hat{y}_i)), V(GMM_2(\hat{y}_i)), ..., V(GMM_{K_{\text{max}}}(\hat{y}_i)))
				\end{equation}
			\item Infine, i valori ottenuti vengono rappresentati graficamente e il modello ottimale viene scelto nel \textit{Knee Point} (KP), cioè dove l'incremento di qualità del modello diventa marginale.
				Questa scelta evita il rischio di \textit{overfitting}\footnote{Fenomeno per cui un modello si adatta eccessivamente ai dati di training, perdendo capacità di generalizzazione}, garantendo al contempo una suddivisione coerente delle regioni di espressione.
		\end{enumerate}
	\item Una volta determinato il modello ottimale, ogni spot $s_j$ viene assegnato alla componente gaussiana con la massima probabilità a posteriori, generando così un insieme di partizioni spaziali
		\begin{equation}
			\mathbb{P}_i = \{P_{i1}, ..., P_{ik}\}
		\end{equation}
		dove ciascun sottogene $P_{im}$ è definito nel seguente modo:
		\begin{equation}
			P_{im} = \left\{ s_j \;\middle|\; m = \arg\max_{\ell} P(\ell \mid \hat{y}_{ij}) \right\}
		\end{equation}
\end{enumerate}
\subsection{Creazione maschere binarie}\label{subsec:CreazioneMaschereBinarie}
Per ogni gene $g_i \in \mathbb{G}$, generiamo dalle partizioni ottenute la media dell'espressione smussata all'interno di ciascuna regione $P_{im}$.
Dopodichè la regione con la media più bassa viene identificata come zona a bassa espressione:
\begin{equation}
	\label{ec:BassaEspressione}
	P_i^{\text{min}} = \arg\min_{P \in P_i} \left(\frac{1}{|P|} \sum_{s_j \in P} \hat{y}_{ij}\right)
\end{equation}
Si definisce quindi una maschera binaria $\overline{\overline{y}}_i \in \{0, 1\}^n$ dove:
\begin{equation}
	\label{eq:BinaryMaskEq}
	\overline{\overline{y}}_{ij} =
		\begin{cases}
			0, & \text{se } s_j \in P_i^{\text{min}} \quad \text{(bassa espressione)}\\[4pt]
			1, & \text{altrimenti.}
		\end{cases}
\end{equation}

Questa maschera sintetizza il comportamento spaziale del gene, 
distinguendo le regioni di minima attività a cui è stato 
assegnato valore $0$, da quelle di maggiore espressione a cui 
è stato assegnato valore $1$.
Le maschere così generate costituiscono la 
rappresentazione compatta e comparabile dei pattern 
spaziali di ciascun gene.
...
I file 10x Visium sono in formato \texttt{.h5}, un file specifico pronto per essere 
elaborato da \texttt{ScanPy}, una libreria Python open-source\footnote{Codice sorgente libero} progettata per l'analisi di dati di trascrittomica.
...
\medskip\noindent
La tabella \ref{tab:tsvTable} è solo una sezione di cosa contiene davvero il file \texttt{.h5}, nella realtà sono presenti svariati gigabyte di dati.

Oltre ai file sopracitati viene fornita anche una cartella 
chiamata \texttt{spatial} contenente immagini ad alta e bassa definizione 
del tessuto e il file delle posizioni degli 
spot, chiamato \texttt{tissue\_positions.csv}
...
Questa moltitudine di funzioni, assieme a velocità non indifferente, ne hanno favorito l'adozione in molti ambiti di ricerca.\cite{scanPyUsage}

Dal punto di vista della manutenibilità invece, Scanpy è sviluppato nell'ambito del progetto \textit{scverse}, 
...
\begin{itemize}
	\item \texttt{sc.pp.*} riguarda le varie funzioni per il pre-processing.
...
\begin{itemize}
  ...
  \item \texttt{genome}: Opzionale, è l'identificatore del genoma che si vuole filtrare, se presente nei metadati 10x.
  ...
  \item \texttt{library\_id}: Opzionale, è l'identificativo della libreria Visium. Risulta utile quando occorre concatenare più librerie.
...
Come detto in precedenza, occorre applicare prima di tutto uno smoothing per poter ripulire e ridurre la complessità dei dati ottenuti.
...
Lo Snippet del codice \ref{code:MeanFilter} presenta 3 funzioni 
principali che permettono di utilizzare il filtro di media: la prima, 
\texttt{get\_gene\_data(data, gene\_id)}, permette di 
recuperare dall'oggetto \texttt{AnnData} un gene dato il suo \texttt{id}.
Il suo funzionamento è molto basico in quanto intuitivamente 
viene creata una variabile contenente la matrice trasposta 
di \texttt{AnnData.X}, che, come detto precedentemente, l'oggetto \texttt{X} 
all'interno di \texttt{AnnData} contiene la matrice 
\textit{Features}$\times$\textit{Barcodes}, e rendendola 
trasposta essa viene trasformata in 
\textit{Barcodes}$\times$\textit{Features}, ovvero, anzichè avere 
in ogni riga uno spot e per ogni colonna l'espressione 
genica di un determinato gene su quello spot, avrà 
un gene per ogni riga, e la sua espressione genica su 
ogni spot per ogni colonna.
La tabella \ref{tab:ann_data_and_transpose} contiene una 
rappresentazione visiva della matrice \texttt{AnnData.X} e 
della sua relativa trasposta.
...
La terza funzione, \texttt{opt\_mean\_filter\_iterated(gene\_data, it)}, 
semplicemente itera \texttt{it} volte la funzione precedente.
Questo perchè iterando più volte il filtro di media, si ottiene 
una rappresentazione più accurata del valore dei vicini di ogni gene, 
e quindi una rappresentazione migliore delle zone più dense.
\newline
...
geniche "smussate" dal filtro di media, stimiamo per ogni gene 
un modello di misura  $\forall k \text{ tale che } 1 \le k \le 10$ 
gaussiane, attraverso la funzione di \texttt{GaussianMixture()} della libreria \texttt{sklearn}.
...
Si può osservare dalla figura \ref{fig:gaussianMixture} come con $2 \le k$ la rappresentazione non colga efficacemente ogni picco dell'espressione genica del gene preso in esame; 
si verifica, perciò, un fenomeno di \textit{underfitting}\footnote{Modello troppo semplice per catturare la complessità dei dati}.
Al contrario, con $k \>> 3$ si verifica il problema opposto, ovvero si hanno troppe gaussiane per rappresentare efficacemente i dati in oggetto, 
verificandosi dell'\textit{overfitting}.
...
una volta addestrata la funzione \texttt{sklearn.mixture.GaussianMixture()} con un gene, 
...
La formula con la quale lo si può ottenere è la seguente:
\begin{equation}
...
\begin{lstlisting}[style=mypython,
				   label=code:gmmAndKnee,
				   caption={Funzioni per ottenere BIC e KP},
...
Questo procedimento porterà ad avere un array, di 
lunghezza \texttt{len($n_{\text{spots}}$)} in cui 
si avrà valore $1$ se il gene risulta presente nello spot $s_i$, oppure $0$,  con $1 \le i \le s_n$.

\newpage\noindent
Il codice per generare la maschera binaria è il seguente:
\begin{lstlisting}[style=mypython,
				   label=code:binaryMask,
				   caption={Funzioni per ottenere le maschere binarie una volta ottenuta la k del Knee Point},
				   basicstyle=\ttfamily\scriptsize]
	def fit_MoG(gdata, k):
		data = gdata.reshape(-1, 1)
...
\begin{itemize}
	\item \texttt{fit\_MoG( ... )} riceve il vettore di espressione smussata \texttt{gdata} e il numero di componenti $k$ trovate dal Knee Point. Viene poi trasformata in colonna da \texttt{gdata.reshape(-1,1)} per adattarsi al modello GaussianMixture. Il modello viene addestrato e restituisce i vettori di medie \texttt{mus} e deviazioni standard \texttt{stds} delle gaussiane. 
  	\item \texttt{smallest\_dist( ... )} seleziona la gaussiana con la media più bassa, salvandola nell'indice \texttt{idx}, assumendo che essa rappresenti l'assenza di espressione. Restituisce poi la media $m$ e la deviazione standard $v$ di quella componente.
	\item Nell codice \texttt{gdata > (m + 1.5 * v)).\text{astype(int)}} passato come argomento alla funzione \texttt{expansion\_filter\_iterated}
